...
* To protect VMs on vVol datastores, deploy ONTAP tools for VMware vSphere first. For supported ONTAP tools versions, see the https://imt.netapp.com/matrix/imt.jsp?components=134348;&solution=1517&isHWU&src=IMT[NetApp Interoperability Matrix Tool^]. ONTAP tools provision and configure storage on ONTAP and the VMware web client.

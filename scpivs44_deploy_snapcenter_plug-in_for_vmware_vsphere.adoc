...
* You must have a certificate with valid Public and Private Key files.
...
. On the *Select an OVF template* page, specify the location of the `.tar` file (as shown in the following table) and click *Next*.
+
|===
|If you downloaded the `.tar` file to… |Do this…

|An internet location
|Enter the URL. Supported URL sources are HTTP and HTTPS.
|A local file
|Click *Choose Files* and navigate to the .tar file.
|===
...
+
This step specifies where to import the `.tar` file into vCenter. The default name for the VM is the same as the name of the selected `.tar` file. If you change the default name, choose a name that is unique within each vCenter Server VM folder.
+

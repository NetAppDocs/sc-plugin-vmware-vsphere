...
* You can deploy the SnapCenter VMware plug-in in the same vCenter as the virtual appliance for VSC 7.0U1 and later.
...
. On the *Select an OVF template* page, specify the location of the `.ova` file inside the tar archive file (as shown in the following table) and click *Next*.
+
|===
...
|A local file
|Extract the .tar file, which contains the .ova file onto your local system. Click *Choose Files* and navigate to the .ova file.
|===

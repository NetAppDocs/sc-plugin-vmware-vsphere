...
.Before you begin
 ...
 [Note]
 You can deploy the SnapCenter VMware plug-in in the same vCenter as the virtual appliance for vCenter 7.0U1 and later.
...
* To protect VMs on vVol datastores, you must first deploy ONTAP tools for VMware vSphere. VMware vSphere versions 9.10 and above are supported. ONTAP tools provisions and configures storage on ONTAP and on the VMware web client.

...
summary: 'Use this task to modify the vCenter logon credentials configured in SnapCenter Plug-in for VMware vSphere (SCV). Updating these credentials helps SCV maintain registration and access to vCenter.'
---
...
Use this procedure when you need to register or re-register SCV with vCenter after a vCenter credential change.
When you change the vCenter password, you need to unregister ONTAP tools for VMware vSphere and re-register it with the new password for the vVol backups to work seamlessly.
// Updated for SNAPCDOC-401

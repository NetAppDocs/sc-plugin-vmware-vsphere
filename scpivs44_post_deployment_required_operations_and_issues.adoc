...
= Post-deployment required operations and issues for the SnapCenter Plug-in for VMware vSphere
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
. Clear the browser cache and then check if the user interface is operating properly.
+
...
. Log on to the SnapCenter Plug-in for VMware vSphere management user interface using the format `\https://<appliance-IP-address>:8080`. Use the admin username, password, and the MFA token details to login. MFA token can be generated from the maintenance console.
. Restart the service.

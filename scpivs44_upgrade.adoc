...
= Upgrade from an earlier release of the SnapCenter Plug-in for VMware vSphere
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
[NOTE]
Upgrade to SCV 6.2 is supported only on VMware vCenter server 7 update 1 and later versions, for VMware vCenter server prior to version 7 update 1, you should continue to use SCV 4.7. The upgrade is disruptive on unsupported versions of VMware vCenter server.
...
|Upgrade to SCV 6.2
|SCV 6.0
|Upgrade to SCV 6.1 and SCV 6.2
|SCV 5.0
...
.. On the vCenter user interface, select *vSphere client (HTML)*.

...
[NOTE]
The default value for retaining job logs is 30 days; the default value for retaining jobs is 90 days. Job logs and jobs that are older than the configured retention are purged every six hours. #You can use the Configuration REST APIs to modify how long jobs and job logs are retained.# You cannot modify the purge schedule.

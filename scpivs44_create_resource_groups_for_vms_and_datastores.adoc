...
= Create resource groups for VMs and datastores in the SCV
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
For more information, refer to the ONTAP tools for VMware vSphere documentation. For supported versions, visit  https://imt.netapp.com/imt/imt.jsp?components=180121;&solution=1517&isHWU&src=IMT[NetApp Interoperability Matrix Tool^].
...
* For FlexGroup volumes in NetApp MetroCluster environments, if you are using ONTAP 9.8 or 9.9, restart the SnapCenter Plug-in for VMware vSphere service and resynchronize SnapMirror relationships after a switchover or switchback before backing up resource groups. In ONTAP 9.8, backups might hang after a switchback; this is resolved in ONTAP 9.9.
...
Always refer to https://imt.netapp.com/imt/imt.jsp?components=180121;&solution=1517&isHWU&src=IMT[NetApp Interoperability Matrix Tool (IMT)^] for the latest information about SnapCenter support.

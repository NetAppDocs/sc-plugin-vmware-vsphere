...
Resource groups can contain any combination of VMs and datastores. Beginning with SnapCenter Plug-in for VMware vSphere 4.4, resource group can also contain one FlexGroup volume or one or more FlexVol volumes, but not both.
...
. In the left Navigator pane of the VMware vSphere web client, click *Resource Groups*, then click *Create* to start the wizard.
+
...
|Beginning with SnapCenter Plug-in for VMware vSphere 4.4, if you want the suffix “_recent” added to the latest Snapshot copy, then check this box.
...
* The default Snapshot copy names use the format
`<ResourceGroup>_<HostName>_<TimeStamp>`
...
* VMDKs are on unsupported storage; for example, on an ONTAP system running in
+
7-Mode or on a non-ONTAP device.

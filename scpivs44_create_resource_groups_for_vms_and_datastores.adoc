...
+
Secondary protection involves creating a consistency group in the primary, including all the storage units protected by the resource group. You need to create a consistency group-based SnapMirror relationship in the preferred Cluster and SVM using a specified policy from the primary. The secondary protection is supported only for ASA r2 based datastore and virtual machines. The cluster peering or SVM peering should be pre-configured. Secondary protection only allows asynchronous SnapMirror policies.
...
. On the *Secondary protection*  page, the list of selected resources are displayed with their protection status. To protect the unprotected resources, select the replication policy type and consistency group suffix.
// 6.1 updates 

...
* #To protect vVols VMs (VMs on VMware virtual volumes), you must first deploy ONTAP Tools for VMware (OTV) to provision and configure storage for vVols on ONTAP and on the VMware web client before you can use SnapCenter Plug-in for VMware vSphere to protect the data.#
+
...
|#9.8#
#SnapCenter Plug-in for VMware vSphere uses this to manage vVols datastores (VMware virtual volumes).#
|===
...
The port is used for communication between the #storage VM host for SnapCenter Plug-in for VMware vSphere and vCenter when performing VM and datastore backup and restore operations.#
// Updated for Burt 1435536 3Nov2021 Ronya
#You must use the default port 443 if you plan to protect vVol VMs.#

...
= Learn about the SnapCenter Plug-in for VMware vSphere
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
====
[NOTE]
Enhance VMware protection using NetApp Backup and Recovery, which provides 3-2-1 backup, ransomware protection, automated backup management, improved scalability, reduced costs, and centralized backup management of datastores and VMs across multiple vCenter instances from a single interface. You can migrate SCV policies, snapshots, and schedules to NetApp Backup and Recovery and manage all VM protection from a single console. For more information, refer to link:https://docs.netapp.com/us-en/data-services-backup-recovery/[NetApp Backup and Recovery documentation].
====

SnapCenter Plug-in for VMware vSphere is a standalone virtual appliance (Open Virtual Appliance format) that provides data protection services for VMs and datastores, and supports data protection services for SnapCenter application-based plug-ins. This document describes how to deploy and use SnapCenter Plug-in for VMware vSphere and includes quick start information. SnapCenter Plug-in for VMware vSphere is deployed as a Linux-based virtual appliance.

...
Audit logs are stored within the appliance and are periodically verified for integrity. Event forwarding allows the you to obtain events from the source or forwarding computer and store it in a centralized computer, which is the Syslog Server. Data is encrypted in transit between the source and the
destination. 
...
Log in to the SnapCenter Plug-in for VMware vSphere to perform this operation.

.Steps

. Login to the SnapCenter Plug-in for VMware vSphere.
. From the Dashboard select *Settings* > *Audit Logs* > *Audit Settings*.
. In the *Audit settings* pane, enter the following details:
+
* Syslog Server Host 
* Syslog Server Port 
* RFC format
...
. Login to the SnapCenter Plug-in for VMware vSphere.
. From the Dashboard select *Settings* > *Audit Logs* > *Settings*.
. In the *Audit settings* pane, you can set new values for maximum rollover files and audit log size.

...
. Log in to the SnapCenter Plug-in for VMware vSphere.
. In the left navigation pane, select *Settings* > *Audit Logs* > *Settings*.
. In the *Audit Log Settings* pane, enter the *Number of audit entries*, which is the maximum number of audit log files and *Audit log size limit*, which is the audit log file size limit. 
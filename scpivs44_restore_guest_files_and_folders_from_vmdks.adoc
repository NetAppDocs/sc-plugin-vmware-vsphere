...
. Click *Menu* and select the *Hosts and Clusters* menu option, then select a VM, then select *SnapCenter Plug-in for VMware vSphere*, and then click *Guest File Restore*.
. In the VMware vSphere client, click *Guest File Restore* from the secondary drop-down list to start the wizard.

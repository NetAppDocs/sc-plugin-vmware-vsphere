...
[.lead]
You can download the `.tar` file for SnapCenter Plug-in for VMware vSphere from the NetApp Support Site.

The `.tar` file includes a set of microservices for VM and datastore data protection, which are performed by the SnapCenter VMware plug-in. When the deployment is complete, all components are installed on a Linux VM in your environment.

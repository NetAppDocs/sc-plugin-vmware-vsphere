...
* Attach and restore operations connect VMDKs using the default SCSi controller. When VMDKs that are attached to a VM with a NVMe controller are backed up, the attach and restore operations use NVMe controller.

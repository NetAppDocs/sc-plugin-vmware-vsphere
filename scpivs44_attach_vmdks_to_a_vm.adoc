...
You need to add NVMe Controller to the VM before attaching a disk.


.Steps

. In the VMware vSphere client GUI, click *Menu* in the toolbar, and then select *Hosts and clusters* from the drop-down list.
. In the left navigation pane, right-click a VM, then select *SnapCenter Plug-in for VMware vSphere* in the drop-down list, and then select *Attach virtual disk* in the secondary drop-down list.

...
[.lead]
You can attach one or more VMDKs from a backup to the parent VM, or to an alternate VM on the same ESXi host. #Beginning in SnapCenter Plug-in VMware vSphere 4.5, you can attach to an alternate VM on an alternate ESXi host managed by the same vCenter in linked mode.#

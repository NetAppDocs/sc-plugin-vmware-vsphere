...
If you are using VMFS datastore with NVMe over TCP protocol, then it is mandatory to add NVMe Controller to the VM before attaching a disk. Follow the steps below to add NVMe controller to the VM.

. Log in to the vCenter client
. Select the VMFS datastore
. Right-click on the datastore and go to *Edit settings*
. In the edit settings window, select *Add new device* > *NVMe Controller*
. Select *Add new device* > *Hard disk*
. In the *Virtual Device Node* field, select *New NVMe controller* and click *OK*.

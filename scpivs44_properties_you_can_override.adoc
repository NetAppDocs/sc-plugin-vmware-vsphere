...
+
Specifies to disable all weakCiphers, except those listed in  `include.weakCiphers`, for the communication channel between SnapCenter Plug-in for VMware vSphere and SnapCenter.
...
* *include.weakCiphers*
+
When `disable.weakCiphers` is set to `true`, specifies the weak ciphers that you do not want to be disabled. The following weak ciphers are usually excluded from the disable operation:
+
`include.weakCiphers=`TLS_RSA_WITH_AES_256_CBC_SHA256, TLS_RSA_WITH_AES_256_CBC_SHA256,TLS_DHE_RSA_WITH_AES_256_CBC_SHA256, TLS_RSA_WITH_AES_128_CBC_SHA256, TLS_DHE_RSA_WITH_AES_128_CBC_SHA256, TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA384, TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256, TLS_RSA_WITH_AES_128_GCM_SHA256, TLS_RSA_WITH_AES_256_GCM_SHA384`

...
+
If no values were assigned to the secondary snapshot locking period during policy creation, it uses the values of the primary snapshot locking period. The underscore in the secondary snapshot locking period indicates that it is the same as the primary snapshot locking period.
